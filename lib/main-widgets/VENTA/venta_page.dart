...
class VentaPage extends StatefulWidget {
  ...
  const VentaPage({super.key, this.firstName, this.emailUser, this.inventarioId});
...
class _VentaPageState extends State<VentaPage> {
  ...
  Future<void> _verificarInventario() async {
    ...
    } catch (e) { print("Error de inventario: $e"); }
  }
  ...
  Future<Map<String, dynamic>> _getDatosCalzado(String calzadoId) async {
    ...
    final snap = await FirebaseFirestore.instance.collection('calzado').doc(calzadoId).get();
    Map<String, dynamic> result = snap.exists ? snap.data()! : {'nombre': 'Sin nombre', 'icono': null};
  ...
  Widget build(BuildContext context) {
    ...
    bool esHoy = DateFormat('dd/MM/yyyy').format(_fechaFiltro) == DateFormat('dd/MM/yyyy').format(DateTime.now());
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Ventas Realizadas', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Georgia')),
        actions: [
          IconButton(icon: const Icon(Icons.today), onPressed: () => setState(() => _fechaFiltro = DateTime.now())),
          IconButton(icon: const Icon(Icons.calendar_month), onPressed: () => _seleccionarFecha(context)),
        ],
      ),
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Text(
                esHoy ? 'Ventas de Hoy: ${DateFormat('dd/MM/yyyy').format(_fechaFiltro)}' : 'Filtrado: ${DateFormat('dd/MM/yyyy').format(_fechaFiltro)}',
                textAlign: TextAlign.center,
                style: TextStyle(color: esHoy ? Colors.green.shade900 : Colors.blue.shade900, fontWeight: FontWeight.bold),
              ),
            ),
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                    return Center(child: Text(esHoy ? 'Aún no hay ventas registradas hoy.' : 'No hay ventas para esta fecha.'));
                  }
      ...
      floatingActionButton: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          _buildFab(Designwidgets().linearGradientBlue(context), "btn1", _navegarFormulario, "Código"),
          const SizedBox(height: 12),
          _buildFab(Designwidgets().linearGradientPurple(context), "btn3", _navegarFormularioMuestra, "Muestra"),
          const SizedBox(height: 12),
          _buildFab(Designwidgets().linearGradientFire(context), "btn2", _navegarFormularioMultiple, "Por Mayor"),
        ],
      ),
    );
  }
  ...
  Widget _miniChip(String texto, Color color) {
    return Container(
      ...
      child: Text(texto, style: TextStyle(fontSize: 10, color: color, fontWeight: FontWeight.bold)),
    );
  }
      ...
      builder: (context, calzadoSnap) {
        ...
        return Card(
          ...
          child: ExpansionTile(
            ...
            title: Text(calzadoData['nombre'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            subtitle: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Text('Cant: ${filaData['cantidad']} • Talla: ${filaData['talla']}', style: const TextStyle(fontSize: 13, color: Colors.black54)),
                    Text('S/ ${precioTotal.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green, fontSize: 15)),
                  ],
                ),
                Padding(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      if (filaData['plataforma'] != null && filaData['plataforma'] != '' && filaData['plataforma'] != false) _miniChip('Plat: ${filaData['plataforma']}', Colors.blue),
                    ],
                  ),
                ),
                const SizedBox(height: 10),
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(child: Text(_formatFechaLarga(filaData['fecha_creacion']), style: TextStyle(fontSize: 11, color: Colors.grey.shade800, fontWeight: FontWeight.w600))),
                    ],
                  ),
                ),
              ],
            ...
            children: [
              ...
              _buildDetalleRow(Icons.payments, 'Método de Pago', filaData['metodo_pago'] ?? 'N/A'),
              _buildDetalleRow(Icons.storefront, 'Lugar de Venta', filaData['lugar_venta'] ?? 'N/A'),
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      TextButton.icon(
                        onPressed: () {
                          Navigator.push(context, MaterialPageRoute(builder: (_) => VentaFormPage(
                      ...
                      TextButton.icon(
                        onPressed: () => _eliminarVentaConReversa(filaId, filaData),

...
class _InventarioFormPageState extends State<InventarioFormPage> {
  ...
  Future<void> _guardarFilaInventario() async {
    ...
    // ✅ Validación de duplicados dentro del formulario
    ...
    // ✅ Validar que la suma de subfilas sea igual a la cantidad principal
    ...
    try {
      if (widget.filaId == null) {
  // 🆕 Crear o fusionar fila existente
  ...
  if (filaExistenteSnap.docs.isNotEmpty) {
    // 🧩 Ya existe fila con el mismo calzado → fusionar
    ...
    for (var sub in _subfilas) {
      ...
      if (subExistente.docs.isNotEmpty) {
        // 🧮 Sumar cantidad a subfila existente
    ...
    // 🆕 No existe fila igual → crear nueva
    ...
    for (var sub in _subfilas) {
      ...
      Navigator.pop(context, true); // ✅ Pop con reload
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          children: [
            ...
            Expanded(
              child: DropdownButtonFormField<int>(
                ...
                onChanged: (v) => setState(() => _subfilas[index]['taco'] = v ?? 1),
              ),
            ),
          ],
        ),
